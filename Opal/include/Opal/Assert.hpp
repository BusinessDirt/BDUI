...
#include <filesystem>
    ...
    #define OPAL_INTERNAL_ASSERT_IMPL(type, check, msg, ...) { if(!(check)) { MX##type##ERROR(msg, __VA_ARGS__); OPAL_DEBUGBREAK(); } }
    ...
    #define OPAL_INTERNAL_ASSERT_GET_MACRO(...) OPAL_INTERNAL_ASSERT_GET_MACRO_NAME(__VA_ARGS__, MX_INTERNAL_ASSERT_WITH_MSG, MX_INTERNAL_ASSERT_NO_MSG)
