...
namespace Mixture::Vulkan
{
    ...
    {
        Unmap();
        vkDestroyBuffer(Context::Get().m_Device->GetHandle(), m_Buffer, nullptr);
    ...
    VkResult Buffer::Map(VkDeviceSize size, VkDeviceSize offset)
    {
        ...
        return vkMapMemory(Context::Get().m_Device->GetHandle(), m_Memory, offset, size, 0, &m_Mapped);
    }
    ...
    VkResult Buffer::Flush(VkDeviceSize size, VkDeviceSize offset)
    {
        ...
        return vkFlushMappedMemoryRanges(Context::Get().m_Device->GetHandle(), 1, &m_MappedRange);
    }
    ...
    VkResult Buffer::Invalidate(VkDeviceSize size, VkDeviceSize offset)
    {
        ...
        m_MappedRange.offset = offset;
        m_MappedRange.size = size;
        return vkInvalidateMappedMemoryRanges(Context::Get().m_Device->GetHandle(), 1, &m_MappedRange);
    }
    ...
    void Buffer::Create(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
    {
        ...
        VK_ASSERT(vkCreateBuffer(Context::Get().m_Device->GetHandle(), &bufferInfo, nullptr, &buffer), "Failed to create VkBuffer");

        VkMemoryRequirements memRequirements;
        vkGetBufferMemoryRequirements(Context::Get().m_Device->GetHandle(), buffer, &memRequirements);

        VkMemoryAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        allocInfo.allocationSize = memRequirements.size;
        allocInfo.memoryTypeIndex = Context::Get().m_Device->FindMemoryType(memRequirements.memoryTypeBits, properties);

        VK_ASSERT(vkAllocateMemory(Context::Get().m_Device->GetHandle(), &allocInfo, nullptr, &bufferMemory), "Failed to allocate buffer memory");

        vkBindBufferMemory(Context::Get().m_Device->GetHandle(), buffer, bufferMemory, 0);
    }
